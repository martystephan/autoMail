...
interface FilePickerProps {
  ...
  accept?: string;
  multiple?: boolean;
  // Folder picker (webkitdirectory). Dropped folders are walked recursively.
  directory?: boolean;
  disabled?: boolean;
...
export default function FilePicker({
  ...
  const emitFiles = (files: File[]) => {
    if (files.length > 0) onFiles(multiple || directory ? files : files.slice(0, 1));
  };
      ...
      <span className="min-w-0">
        ...
        <span className={`block text-xs ${hasSelection ? "text-green-700/70" : "text-neutral-500"}`}>
          {hasSelection
            ...
            : (hint ?? `Click to browse or drag & drop${directory ? " a folder" : ""} here`)}
        </span>
      </span>
      <input
        ref={inputRef}
